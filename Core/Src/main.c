...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	if(huart->Instance == USART1) {
		HAL_UART_Receive_IT (&huart1, LEDdata, 512);
	}
}
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		*/
		...
		while(TX_complete == 0x00);
		TX_complete = 0;
		PL9823_sendchannelPreset(LEDdata+32*4, LED_per_strips, 1);
		HAL_Delay(DELAY_BETWEEN_SEND);
		// CHANNEL 2 
		while(TX_complete == 0x00);
		TX_complete = 0;
		PL9823_sendchannelPreset(LEDdata+32*5, LED_per_strips, 2);
		HAL_Delay(DELAY_BETWEEN_SEND);
		// CHANNEL 3
		while(TX_complete == 0x00);
		TX_complete = 0;
		PL9823_sendchannelPreset(LEDdata+32*6, LED_per_strips, 3);
		HAL_Delay(DELAY_BETWEEN_SEND);
		// CHANNEL 4 
		while(TX_complete == 0x00);
		TX_complete = 0;
		PL9823_sendchannelPreset(LEDdata+32*7, LED_per_strips, 4);
		HAL_Delay(DELAY_BETWEEN_SEND);
